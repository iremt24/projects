...
def lightbot(commands, level):
    ...
    for k in range(len(commands)):
        if commands[k] == "@":
            ...
            x, y = i, j
            can_move = False
            if direction == "↑" and i > 0 and abs(terrain[i-1, j] - terrain[i, j]) == 1:
                i, j = i-1, j
                A[i, j] = "↑"
                can_move = True
            elif direction == "↓" and i < terrain.shape[0] - 1 and abs(terrain[i+1, j] - terrain[i, j]) == 1:
                i , j = i+1, j
                A[i, j] = "↓"
                can_move = True
            elif direction == "→" and j < terrain.shape[1] - 1 and abs(terrain[i, j+1] - terrain[i, j]) == 1:
                i, j = i, j+1
                A[i, j] = "→"
                can_move = True
            elif direction == "←" and j > 0 and abs(terrain[i, j-1] - terrain[i, j]) == 1:
                i , j = i, j-1
                A[i, j] = "←"
                can_move = True
            else: 
